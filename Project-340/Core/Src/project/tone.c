...
void play_tones() {
    ...
    GPIO_Init();     // Initialize GPIO for DAC control
    DAC_Init();      // Initialize DAC
    DAC_Start();     // Start DAC operation

    for (uint8_t i = 0; i < MAX_TONES; i++) {
        if (tones[i].duration > 0 && tones[i].power > 0) {
            uint32_t half_period = tones[i].duration >> 1;  // Calculate half period for the square wave
            uint32_t start_time = get_count();

            while ((get_count() - start_time) < tones[i].duration) {
                uint32_t current_time = get_count();
                uint32_t elapsed_time = current_time - start_time;
                uint32_t count = elapsed_time % tones[i].duration;

                if (count < half_period) {
                    DAC_Set_Value(2048);  // Set DAC to high value
                } else {
                    DAC_Set_Value(0);    // Set DAC to low value
                }
            }
            DAC_Set_Value(0);  // Ensure DAC is reset to 0 to avoid clicks at the end of the tone
        }
    }
}
